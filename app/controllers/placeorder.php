...
function generate_new_transaction_number() {
	...
	if ($payment_mode_id == 1) {
		...
		//create a new order
		$sql = "INSERT INTO orders(users_id, transaction_code, purchase_date, status_id, payment_mode_id) VALUES ('$user_id', '$transaction_number', '$purchase_date', '$status_id', '$payment_mode_id'); ";
	...
	$staff_email = 'littlemisskdress4u@gmail.com';
	$customer_email = $_SESSION['user']['email'];          //
	$subject = 'Little Miss K - Order Confirmation';
	...
	try {
	    ...
	    $mail->Password = 'littlemissk';                     // SMTP password
	    $mail->SMTPSecure = 'tls';                            // Enable TLS encryption, `ssl` also accepted
	    $mail->Port = 587;                                    // TCP port to connect to

	    //Recipients
	    $mail->setFrom($staff_email, 'LittleMissK');
    ...
    $transaction ->setAmount($amount)
                ...
                ->setDescription('Payment for dress4u Purchase')
                ->setInvoiceNumber(uniqid("dress4u_"));
